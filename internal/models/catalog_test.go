...
func TestSetCatalog_ListModelsIncludesDiscovered(t *testing.T) {
	...
	// gpt-9.9-nova is deliberately absent from the built-in table: a non-claude
	// ID that only discovery could introduce, so the assertion below tests
	// SetCatalog's filter rather than the built-in GPT rows.
	SetCatalog([]CatalogModel{
		{ID: "claude-opus-9", MaxInputTokens: 1_000_000},
		{ID: "gpt-9.9-nova", MaxInputTokens: 272_000},
	})
	...
	if slices.Contains(ids, "gpt-9.9-nova") {
		t.Errorf("ListModels() = %v, must not include discovered non-claude models", ids)
	}
